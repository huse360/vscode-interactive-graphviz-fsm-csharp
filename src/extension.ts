...
import MachineParser from "./StateMachine/MachineParser";
...
function swiftDot(src: string) : string {
  const parser = new MachineParser();
...
function onActivate(context: vscode.ExtensionContext) {
  ...
  vscode.workspace.onDidChangeTextDocument((event) => {
    if (event.document.languageId === DOT
      ...
      if (panel) {
        ...
        panel.requestRender(swiftDot(event.document.getText()));
  ...
  context.subscriptions.push(
    vscode.workspace.onDidOpenTextDocument((doc) => {
      ...
      vscode.commands.executeCommand("fsm-swift-interactive-preview.preview.beside", {
        document: doc,
      });
    }),
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("fsm-swift-interactive-preview.preview.beside", (a) => {
      ...
      if (!options.content) {
        if (options.document) {
          ...
          options.content = swiftDot(options.document.getText());
        } else if (options.uri) {
